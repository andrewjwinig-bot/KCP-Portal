...
type Bar = { year: number; value: number | null; budget: number | null; projected: boolean; monthsCovered: number; next?: boolean };

export function HistoryBars({ years, forecast, budget }: {
  years: LineYear[];
  /** The latest year's full-year reprojection, when that year is partial. */
  forecast?: number | null;
  /** The year being budgeted, drawn as the last bar so it reads against the
   *  history — kept OUT of the average, which describes what happened. */
  budget?: { year: number; value: number } | null;
}) {
  ...
  if (budget) bars.push({ year: budget.year, value: budget.value, budget: null, projected: false, monthsCovered: 12, next: true });
  ...
  const inAvg = bars.filter((b) => !b.next && b.value != null && (b.projected || b.monthsCovered >= 12));
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }} onMouseLeave={() => setHover(null)}>
          ...
          return (
            <g key={b.year}>
              <rect x={cx(i) - bw / 2} y={top} width={bw} height={Math.max(1, bot - top)} rx={3}
                fill={b.next ? "var(--series-3)" : "var(--series-1)"} fillOpacity={b.projected ? 0.35 : on ? 1 : 0.85}
        ...
        {bars.map((b, i) => (
          <text key={`x${b.year}`} x={cx(i)} y={H - PAD.b + 18} fontSize={12} fontWeight={700} textAnchor="middle" fill="var(--text)">
            {b.year}{b.next ? " budget" : b.projected ? " (proj.)" : b.value != null && b.monthsCovered < 12 ? ` · ${b.monthsCovered} mo` : ""}
          </text>
        ))}
