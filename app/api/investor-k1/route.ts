      ...
      owners: PROPERTY_OWNERSHIP.filter((p) => p.hasK1Distribution).flatMap((p) =>
        p.owners.map((o) => {
          ...
          return {
            ...
            email: r.email, alsoEmail: r.alsoEmail, recipientNames: r.recipientNames, emailSource: r.source, emailNote: r.note,
          };
        }),
      ),
    });
      ...
      interests: await Promise.all(interests.map(async ({ propertyCode, hasK1, owner }) => {
        ...
        return {
          ...
          ...(() => {
            ...
            return { email: r.email, alsoEmail: r.alsoEmail, recipientNames: r.recipientNames, emailSource: r.source, emailNote: r.note };
          })(),
          documents: docs
