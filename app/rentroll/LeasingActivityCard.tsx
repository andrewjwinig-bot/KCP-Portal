...
function tenantOptions(rentroll: RentRollData | null): { unitRef: string; label: string; tenant: string; building: string; sqft: number }[] {
  if (!rentroll) return [];
  const out: { unitRef: string; label: string; tenant: string; building: string; sqft: number }[] = [];
  for (const p of rentroll.properties) {
    const def = PROPERTY_DEFS.find((d) => d.id.toUpperCase() === p.propertyCode.toUpperCase());
    const buildingName = def?.name ?? p.propertyCode;
...
export default function LeasingActivityCard({ rentroll }: { rentroll: RentRollData | null }) {
  ...
  function addVacating()       { update("tenantsVacating", (r) => [...r, { id: uid(), unitRef: "", expirationDate: "" } as TenantVacating]); }
  function addOption()         { update("optionsToRenew",  (r) => [...r, { id: uid(), unitRef: "", term: "", noticeDate: "", optionTermExp: "" } as OptionToRenew]); }
          ...
          <tbody>
            {data.prospects.map((p) => (
              <tr key={p.id}>
                ...
                <td style={tableCellStyle}>
                  <input style={inputStyle} value={p.building} onChange={(e) => update("prospects", (r) => r.map(x => x.id === p.id ? { ...x, building: e.target.value } : x))} />
                </td>
                ...
                <td style={{ ...tableCellStyle, textAlign: "right" }}>
                  <input style={{ ...inputStyle, textAlign: "right", width: 60 }} value={p.rating ?? ""} onChange={(e) => {
                    const v = e.target.value.replace(/[^0-9]/g, "").slice(0, 1);
                    const n = v === "" ? null : Math.max(1, Math.min(5, Number(v)));
                ...
                <tr key={v.id}>
                  <td style={tableCellStyle}>
                    <select
                      style={inputStyle}
                      value={v.unitRef}
                      onChange={(e) => update("tenantsVacating", (r) => r.map(x => x.id === v.id ? { ...x, unitRef: e.target.value } : x))}
                    >
                      <option value="">— Pick a tenant —</option>
                      {opts.map((o) => <option key={o.unitRef} value={o.unitRef}>{o.label}</option>)}
                    </select>
                  </td>
                  <td style={tableCellStyle}><span className="muted small">{opt?.building ?? "—"}</span></td>
                  <td style={{ ...tableCellStyle, textAlign: "right" }}><span className="muted small">{opt ? opt.sqft.toLocaleString() : "—"}</span></td>
                  <td style={tableCellStyle}>
                ...
                <tr key={o.id}>
                  <td style={tableCellStyle}>
                    <select
                      style={inputStyle}
                      value={o.unitRef}
                      onChange={(e) => update("optionsToRenew", (r) => r.map(x => x.id === o.id ? { ...x, unitRef: e.target.value } : x))}
                    >
                      <option value="">— Pick a tenant —</option>
