...
function UnitsTable({ units, propertyCode, hideNNN, tenantMeta, onBaseYearChange, vacatingUnitRefs }: {
  ...
  // Hidden entirely for the SERVICE persona — they don't need lease financials.
  const showBaseYear = !hideRent && (JV_III_CODES.has(upperCode) || NI_LLC_CODES.has(upperCode));
      ...
      <div className="tableWrap" style={{ marginTop: 0 }}>
        <table>
          <thead>
            {!hideRent && (
              <tr>
                <th colSpan={6 + (showBaseYear ? 1 : 0)} style={{ border: "none", background: "transparent" }} />
                <th colSpan={hideNNN ? 2 : 5} style={{ textAlign: "center", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em", color: "var(--muted)", borderBottom: "1px solid var(--border)", paddingBottom: 3 }}>Monthly</th>
              </tr>
            )}
            <tr>
              <th>Tenant</th>
              ...
              {!hideRent && !hideNNN && <th style={{ textAlign: "right" }}>RET</th>}
              {!hideRent && <th style={{ textAlign: "right" }}>Gross</th>}
            </tr>
          </thead>
          <tbody>
...
export default function RentRollPage() {
  ...
  return (
    ...
    <main>
      ...
      {/* The "units skipped — unrecognized property code" banner was removed
          at the owner's request: those rows are entity/clearing and co-working
          lines (2000-*, 4901-CIP*, 08xx pads) the portal deliberately does not
          track, so the banner was permanent noise. `unknownUnits` is still
          computed on import if it is ever needed again. */}
