...
- **Property-wide insurance pool** is a single building figure (`RetailExpensePool.insAmount`). Recon-time corrections to it are PROPERTY-WIDE and live in `lib/cam/retail/poolStore.ts` (keyed by `<property>-<year>`), edited on the CAM Reconciliation page (`InsurancePoolCard`) — NOT per tenant. Insurance is edited ONLY at the property level. The per-tenant `CamConfig.insAmountOverride` is no longer exposed on the unit page (the "Manual Insurance" UI was removed); the field still exists in the data model and is honored from the **seed only** for a genuine outparcel billed on its own liability figure (e.g. Wawa `2300-1883`), so its INS keeps tying to the workbook. Don't re-add a per-tenant insurance UI without revisiting this.
