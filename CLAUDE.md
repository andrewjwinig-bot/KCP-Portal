...
- **Property-wide insurance pool** is a single building figure (`RetailExpensePool.insAmount`). Recon-time corrections to it are PROPERTY-WIDE and live in `lib/cam/retail/poolStore.ts` (keyed by `<property>-<year>`), edited on the CAM Reconciliation page (`InsurancePoolCard`) — NOT per tenant. The per-tenant `CamConfig.insAmountOverride` on the unit page is **outparcel-only** (a tenant billed on its own liability figure, e.g. Wawa) and still wins for that tenant via the assemble/compute precedence. Don't use the per-tenant field to set the building's insurance.
